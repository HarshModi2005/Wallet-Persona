import { WalletDetails, NFT, Transaction } from '../types/wallet.types';
...
export interface NFTPreferenceProfile {
...
export class NFTRecommendationService {
  ...
  async generateNFTRecommendations(
    ...
    try {
      ...
      const uniqueRecommendations = this.deduplicateAndScore(
        allRecommendations,
        mergedPreferences
      );
   ...
   */
  private async analyzeUserNFTProfile(walletDetails: WalletDetails): Promise<NFTPreferenceProfile> {
  ...
  private async getTrendingRecommendations(
    preferences: NFTPreferenceProfile
  ): Promise<NFTRecommendation[]> {
    try {
   ...
   */
  private async getSimilarStyleRecommendations(
    userNFTs: NFT[],
    preferences: NFTPreferenceProfile
  ): Promise<NFTRecommendation[]> {
    ...
    try {
      ...
      return similarCollections
        ...
        .map(collection => ({
          ...
          category: 'similar_style' as const,
   ...
   */
  private async getUndervaluedRecommendations(
    preferences: NFTPreferenceProfile
  ): Promise<NFTRecommendation[]> {
    try {
      ...
      return marketData
        .filter(nft => 
          nft.floorPrice >= preferences.budgetRange.min &&
          nft.floorPrice <= preferences.budgetRange.max
        )
        .map(nft => ({
          contractAddress: nft.contractAddress,
   ...
   */
  private async getAIPoweredRecommendations(
    walletDetails: WalletDetails,
    preferences: NFTPreferenceProfile
  ): Promise<NFTRecommendation[]> {
    try {
      ...
      const aiResponse = await this.geminiAI.analyzeTransactions(
        walletDetails.transactions || [],
        walletDetails.address,
        walletDetails.profile || { address: walletDetails.address, totalTransactions: 0, totalNftsHeld: 0, uniqueNftCollectionsCount: 0, topNftCollections: [] },
        walletDetails.nfts || [],
        walletDetails.balance || { native: '0', usdValue: 0, totalTokenUsdValue: 0, grandTotalUsdValue: 0 },
        walletDetails.tokens || [],
        walletDetails.historicalActivity || [],
        walletDetails.keyEvents || []
      );
      ...
      const enrichedRecommendations = await Promise.all(
        aiRecommendations.map(async (rec) => {
          ...
          return {
            ...rec,
            floorPrice: marketData?.floorPrice || rec.floorPrice,
  ...
  private buildAIRecommendationPrompt(
    ...
    const userCollections = walletDetails.nfts?.map(nft => nft.collectionName).join(', ') || 'None';
   ...
   */
  private async fetchTrendingNFTs(): Promise<TrendingNFTData[]> {
    ...
    try {
      ...
      return response.data.collections?.map((collection: any) => ({
        ...
        priceChange24h: collection.stats?.one_day_change || 0,
        holders: collection.stats?.num_owners || 0,
        totalSupply: collection.stats?.total_supply || 0,
        marketCap: (collection.stats?.floor_price || 0) * (collection.stats?.total_supply || 0),
   ...
   */
  private calculateTrendingScore(nft: TrendingNFTData, preferences: NFTPreferenceProfile): number {
  ...
  private assessRiskLevel(nft: any): 'low' | 'medium' | 'high' {
    const volume = nft.volume24h || 0;
    const priceChange = Math.abs(nft.priceChange24h || 0);

    if (volume > 50 && priceChange < 5) return 'low';
    if (volume > 10 && priceChange < 20) return 'medium';
    return 'high';
  }
   ...
   * Helper methods
   */
  private extractNFTTransactions(transactions: Transaction[]): Transaction[] {
    return transactions.filter(tx => 
      tx.raw?.logs?.some((log: any) => 
  ...
  ): 'holder' | 'flipper' | 'mixed' {
    const ratio = totalTransactions / Math.max(1, uniqueCollections);
    if (ratio > 3) return 'flipper';
    if (ratio < 1.5) return 'holder';
    return 'mixed';
  }
  ...
  private determineRiskTolerance(
    ...
    if (avgPrice < 0.5 && priceVariance < 0.1) return 'conservative';
    if (avgPrice > 2 || priceVariance > 1) return 'aggressive';
    return 'moderate';
  }
  ...
  private async analyzeCollectionCategories(nfts: NFT[]): Promise<string[]> {
    // Simplified implementation - in production, use AI or collection metadata
    const categories = new Set<string>();
    
    nfts.forEach(nft => {
      const name = nft.collectionName?.toLowerCase() || '';
      if (name.includes('punk') || name.includes('ape')) categories.add('pfp');
      if (name.includes('art') || name.includes('generative')) categories.add('art');
      if (name.includes('game') || name.includes('meta')) categories.add('gaming');
      if (name.includes('land') || name.includes('world')) categories.add('metaverse');
    });

    return Array.from(categories);
  }

  private async analyzeArtStyles(nfts: NFT[]): Promise<string[]> {
    // Simplified implementation - in production, use image analysis AI
    return ['digital', 'abstract', 'realistic']; // Placeholder
  }

  private categorizeCollection(collection: any): string {
    const name = collection.name?.toLowerCase() || '';
    if (name.includes('punk') || name.includes('ape') || name.includes('avatar')) return 'pfp';
    if (name.includes('art') || name.includes('canvas')) return 'art';
    if (name.includes('game') || name.includes('card')) return 'gaming';
    if (name.includes('land') || name.includes('plot')) return 'metaverse';
    return 'utility';
  }
  ...
  private deduplicateAndScore(
    ...
    const uniqueRecs = new Map<string, NFTRecommendation>();
   ...
   */
  private getFallbackRecommendations(): NFTRecommendation[] {
    return [
      {
  ...
  private async findSimilarCollections(userCollections: string[]): Promise<any[]> {
    // Implementation would use similarity algorithms or external APIs
    return [];
  }

  private async analyzeMarketForUndervaluedNFTs(): Promise<TrendingNFTData[]> {
    // Implementation would analyze price history, fundamentals, etc.
    return [];
  }

  private async fetchCollectionData(contractAddress: string): Promise<any> {
  ...
  private calculateSimilarityScore(collection: any, preferences: NFTPreferenceProfile): number {
  ...
  private calculateUndervaluedScore(nft: TrendingNFTData, preferences: NFTPreferenceProfile): number {
  ...
  private extractNFTRecommendationsFromAI(aiResponse: any): any[] {
