...
import { Transaction } from '../types/wallet.types'; // Removed WalletProfile, not directly used here

// Interface for the analysis result, can be expanded for each phase
export interface CoreTransactionAnalysis {
  ...
  transactionCountInDateRange?: number; // To explicitly show how many tx were in range
  ...
  topInteractedAddresses?: { address: string; count: number }[]; // Top 5
}
...
export class TransactionAnalyzerService {
  ...
  public analyzeTransactions(
    ...
    if (startDate || endDate) {
      ...
      console.log(`[TransactionAnalyzerService] Applied date filter: ${startDate?.toISOString().split('T')[0] || 'N/A'} to ${endDate?.toISOString().split('T')[0] || 'N/A'}. ${filteredTransactions.length} of ${transactions.length} txs selected using optimized slice.`);
    }
    ...
    const getLcAddress = (field: any): string | null => {
      if (!field) return null;
      if (typeof field === 'string') return field.toLowerCase();
      // Check for Moralis EvmAddress-like object
      if (field.lowercase && typeof field.lowercase === 'string') return field.lowercase;
      // Fallback for objects with a value property that is a string (e.g. from older SDKs or direct _value access)
      if (field.value && typeof field.value === 'string') return field.value.toLowerCase();
      if (field._value && typeof field._value === 'string') return field._value.toLowerCase(); // As seen in some logs
    ...
    const totalInboundTransactions = filteredTransactions.filter((tx) => {
      const lcTo = getLcAddress(tx.raw?.to);
      const lcFrom = getLcAddress(tx.raw?.from);
    ...
    const totalContractCreationTransactions = filteredTransactions.filter((tx) => {
      ...
      return isToEffectivelyNullOrZero && tx.raw?.receiptContractAddress; // receiptContractAddress indicates success
    }).length;

    const totalFailedTransactions = filteredTransactions.filter(
      (tx) => tx.raw?.receiptStatus === 0 // Corrected to check for number 0
    ).length;

    let cumulativeGasFeesEth = 0;
    let cumulativeGasPriceGwei = 0;
    let successfulTransactionsCountForGasAvg = 0;
    let mostExpensiveTxHash_temp: string | undefined = undefined;
    let mostExpensiveTxFeeEth_temp = 0;

    const txCountByDayOfWeek_temp: { [day: string]: number } = { Mon: 0, Tue: 0, Wed: 0, Thu: 0, Fri: 0, Sat: 0, Sun: 0 };
    const txCountByHourOfDay_temp: { [hour: number]: number } = {};
    for (let i = 0; i < 24; i++) txCountByHourOfDay_temp[i] = 0;

    const uniqueInteractedAddresses_temp = new Set<string>();
    const interactedAddressCounts_temp = new Map<string, number>();

    // Helper to safely get a BigInt value for gas fields
    const getGasBigIntValue = (field: any): bigint | null => {
      if (field === null || field === undefined) return null;
      if (typeof field === 'bigint') return field; // Already a bigint
      ...
      if (typeof field === 'string' || typeof field === 'number') {
        try {
          ...
          if (String(field).trim() === '') return null;
          return BigInt(field);
        } catch (e) {
      ...
      if (typeof field === 'object' && field.value !== undefined && (typeof field.value === 'string' || typeof field.value === 'number')) {
         try {
           if (String(field.value).trim() === '') return null;
      ...
      if (tx.raw && tx.raw.receiptStatus === 1) { // Only process successful transactions for gas
        ...
        if (gasPriceWei_val !== null && gasUsed_val !== null) {
          try {
            ...
            const transactionFeeEth = parseFloat(ethers.formatEther(transactionFeeWei));
    ...
    const analysisResult: CoreTransactionAnalysis = {
  ...
  // Placeholder for a private calculation method
  /*
  private _calculateInboundTransactions(transactions: Transaction[], walletAddress: string): number {
    return transactions.filter(tx => tx.to?.toLowerCase() === walletAddress.toLowerCase()).length;
  }
  */

  // Add other private calculation methods here for each metric
