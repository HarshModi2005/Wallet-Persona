...
import { WalletPersona } from './services/WalletPersonaService';
...
function displayWalletPersona(address: string, details: any, persona: WalletPersona): void {
  ...
  if (persona.recommendations.tokens.length > 0) {
    const tokenList = persona.recommendations.tokens.map(token => 
      `${colors.fg.green}${token}${colors.reset}`
    ).join(', ');
    console.log(`${colors.fg.white}Tokens to Explore:${colors.reset} ${tokenList}`);
  }
  
  if (persona.recommendations.nfts.length > 0) {
    const nftList = persona.recommendations.nfts.map(nft => 
      `${colors.fg.cyan}${nft}${colors.reset}`
    ).join(', ');
    console.log(`${colors.fg.white}NFT Collections:${colors.reset} ${nftList}`);
  }
  
  if (persona.recommendations.dapps.length > 0) {
    const dappList = persona.recommendations.dapps.map(dapp => 
      `${colors.fg.yellow}${dapp}${colors.reset}`
    ).join(', ');
